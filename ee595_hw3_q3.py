# -*- coding: utf-8 -*-
"""
Created on Fri May 11 22:45:49 2018
...
for typez in modelz:    
    ...
    model = Sequential()
    if typez=='VRNN':
        model.add(SimpleRNN(units = 50, activation = 'tanh', input_shape=(1, 1), return_sequences=False))
    elif typez=='LSTM':
        model.add(LSTM(units = 50, activation = 'tanh', input_shape=(1, 1), return_sequences=False))
    elif typez=='GRU':
        model.add(GRU(units = 25, activation = 'tanh', input_shape=(1, 1), return_sequences=False))
    else:
        print('wrong option')
    ...
    trainPredictPlot[1:len(trainHat)+1, :] = trainHat
    #shift test
    testPredictPlot = np.empty_like(dataset)
    testPredictPlot[:, :] = np.nan
    testPredictPlot[len(trainHat):len(dataset)-1, :] = testHat
    '''
